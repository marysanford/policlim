...
capmf_ppeg %>%
  ...
  mutate(across(.cols = ends_with("OBS_VALUE"),
                .fns = ~ lead(.x) - .x,
                .names = "d_{.col}"),
         across(.cols = ends_with("OBS_VALUE"),
                .fns = ~ (lead(.x) - .x)/.x,
                .names = "pc_{.col}"),
         across(.cols = ends_with("OBS_VALUE"),
                .fns = ~ log(.x, base = exp(1)),
                .names = "ln_{.col}")) %>%
  select(REF_AREA, idate, starts_with("d_")) -> pol_change
# Merge
...
capmf_ppeg %>%
  ...
  rename_with(.cols = starts_with("d_LEV"),
              .fn = ~ tolower(paste0("d_",
                                     substr(.x, nchar("d_LEV1_")+1, nchar(.x))))) %>% 
