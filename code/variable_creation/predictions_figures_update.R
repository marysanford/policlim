...
#### Mapping old and new broad climate salience ####
...
predictions %>%
  ...
  # party family
  mutate(party_n = 1) %>%
  group_by(year, parfam_name) %>%
  summarise(salience = mean(final_broad, na.rm = T)) -> predictions_pfy

# Fill holes with zero
cbind(year = c(1990, 1990, 2021, 2021),
      parfam_name = c("AGR", "SIP", "CON", "ETH"),
      salience = c(0, 0, 0, 0)) %>% as.data.frame() %>%
  mutate(year = as.numeric(year), salience = as.numeric(salience)) -> fill
